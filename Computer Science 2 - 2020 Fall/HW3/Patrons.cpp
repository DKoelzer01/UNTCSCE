...
void Patrons::editPatron(Patron* target)
{
	...
	if(tempString != "")
		{
			target->setNumBooks(stoi(tempString,&sz));
		}
}

void Patrons::deletePatron(Patron* target, int id, Loans* loanDB, Books* bookDB)
{
	if(!target->isEmpty())
		{
			if(findId(id))
							...
							for(unsigned i = 0; i < loanDB->getLoanVector()->size(); i++)
								{
									if(target->getId() == loanDB->getLoanVector()->at(i).getLoanId())
										{
											loanDB->checkInBook(loanDB->searchLoan(loanDB->getLoanVector()->at(i).getLoanId()),loanDB->getLoanVector()->at(i).getLoanId());
										}
								}
							for(unsigned i = 0; i < patronVector.size(); i++)
								{
...
void Patrons::menu(Loans* loanDatabase,Books* bookDatabase)
{
	while(true)
		{
			drawMenu();
						...
						case('D'):
							if(patronVector.size() == 0)
							...
							else
								{
									...
									cin >> tempInt;
									temp = searchPatron(tempInt);
									deletePatron(temp,tempInt,loanDatabase,bookDatabase);
								}
							cout << "Press enter to continue ..." << endl;
							cin.ignore(1);
							cin.get();
