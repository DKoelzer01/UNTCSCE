...
pthread_mutex_t indexMutex; //Mutex Lock declaration.
...
void *convertMessage(void *ptr)
{
	...
	pthread_mutex_lock(&indexMutex); //Locks mutex
	if (ind % 2)
	{
	...
	printChar();
	pthread_mutex_unlock(&indexMutex); //Unlocks mutex
	return 0;
}
...
int main()
{
	...
	pthread_mutex_destroy(&indexMutex); //Destroy mutex
